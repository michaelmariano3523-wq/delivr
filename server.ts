...
const CORS_ORIGINS = process.env.CORS_ORIGINS?.split(',') || ['http://localhost:5173', 'http://localhost:3000', 'https://delivr-frontend.vercel.app', 'https://delivr-5x15-git-main-michaelmariano3.vercel.app'];
  ...
  try {
    ...
    app.post('/api/register', async (req, res) => {
      ...
      if (!validateUsername(username)) {
        return res.status(400).json({ error: 'Nome de usuário deve ter 3-30 caracteres (letras, números e underscore)' });
      }
    ...
    app.get('/api/stats/driver/:userId', async (req, res) => {
      try {
        ...
        console.log('Fetching stats for driver:', userId);
        ...
        const earnings = (orders || []).reduce((sum, o) => sum + (o.delivery_fee || 0), 0);
        console.log('Driver stats:', { count: (orders || []).length, earnings });
        ...
        console.error('Exception in driver stats:', e);
    ...
    // WebSockets with enhanced logging and keep-alive
      ...
      ws.on('message', (message) => {
        try {
          ...
          if (data.type === 'update_location') {
            console.log(`[WS] Location update from driver ${data.driverId} for order ${data.orderId || 'none'}:`, { lat: data.lat, lng: data.lng });
